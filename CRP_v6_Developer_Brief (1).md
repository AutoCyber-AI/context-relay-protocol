...
Everything else (SPEC-049 PRM, 050, 051, 053, 055, 057) → **published roadmap**, not built.
...
**Verify the corpus against the repo; bind SPEC-059 and the `crp.Agent` SDK to the real API; ship the reuse layer + structured decoding + streaming governance + one reference agent so anyone can build governed SLM agents by declaring tools and policy; publish the heavy specs as roadmap; keep Comply on the compliance budget line and the position exactly as agreed. Reusability is the whole point.**
