...
This distinction is the spine of both the product and its monetisation: most customers start at Layer 1 (readiness), and convert to Layer 3 (actual compliance) when they ship a production AI system.
...
A defining product integrity choice: some evidence the platform **will not** produce, because regulators expect independence. The pen-test / AI red-team report is **referred out** — and specifically referred to **WASA AI by AutoCyber** for web-application pen-testing and AI-endpoint probing (prompt injection, model extraction, data leakage, refusal bypass). This is a first-party referral within the AutoCyber ecosystem: CRP Comply refers the security testing to WASA AI, the customer uploads the resulting report, and Comply slots it into the Art. 15 accuracy/robustness/cyber evidence. Legal sign-off is similarly referred to the customer's own counsel.

This referral model is both honest (Comply does not fake independent testing) and strategically sound (it routes work to the sibling AutoCyber product, WASA AI).
...
## 7. The BYO-LLM Model (and Monetisation)
...
- **(b) BYOK local** — point at LM Studio / Ollama / vLLM / llama.cpp; nothing leaves the network. Zero token cost. **This is the recommended Free-tier path** ($0 marginal cost).
...
### 7.2 Monetisation — Audited-Call Quota + Prepaid Credits

The live billing model:
...
- **Free tier:** 100 audited calls/month; local-LLM path at $0 marginal cost; local storage only.
- **Prepaid hosted-LLM credits:** when the customer exceeds the monthly tier quota, the platform draws from a prepaid balance instead of blocking. Every new account gets a **$5 welcome bonus (~100 hosted calls)** to experience the full agent before bringing a key. Top-ups: $5 (~100 calls), $20 (~400), $50 (~1000). Stripe-hosted, funds never expire.
- **Paid tiers** (Starter / Scale / Enterprise) unlock: hosted storage, higher quotas, hosted LLM, longer retention.

This is a usage-aligned model: the free tier proves value (the tagline "prove compliance on your first call"), credits prevent hard blocks, and tiers scale with real usage.
...
CRP PROTOCOL (open)  ──── governs every call, emits signals
        ...
        ├─ WASA AI (sibling)        ── pen-test / red-team (referred from Artefact Room)
        └─ CRP COMPLY (this spec)   ── the three-layer compliance platform + revenue
```

CRP Comply is the deployed product where the protocol's governance becomes compliance evidence and a managed programme. WASA AI is the referral partner for independent security testing. CRP Scan feeds the Inbox. The Gateway is the scaled execution layer.

---

## 12. Honest Status & Roadmap

**Live today (Comply 0.1.0):** the three-layer model, recipes + agent, the programme/deliverables/artefacts/evidence/vault surfaces, BYO-LLM with the credit model, the OpenAI-compatible proxy with compliance headers, local/hosted storage, the regulatory corpus, profile-driven tailoring, and the evidence-pack export.
