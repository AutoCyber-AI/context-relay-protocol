    ...
    for key, value in intent.items():
        if key in _INTENT_TO_CAPABILITY:
            ...
            if not (_GROUNDING_RANGE[0] <= value <= _GROUNDING_RANGE[1]):
                refusals.append(f"{key}: must be between {_GROUNDING_RANGE[0]} and {_GROUNDING_RANGE[1]}")
                continue
            ...
            if not isinstance(value, (int, float)):
                refusals.append(f"{key}: must be a number")
                continue
            capabilities.append("safety_budget_multiagent")
            settings["safety_budget_multiagent"] = value
        elif key == "profile":
            if value not in {"balanced", "strict", "medical", "financial"}:
