...
CLERK_AUDIENCE = os.environ.get("CLERK_AUDIENCE", "crp-gateway")
...
async def current_clerk_identity(request: Request) -> Identity:
    ...
    # NOTE: Demo-mode bypasses have been removed. Any `demo` token path must be
    # explicitly re-added behind a compile-time guard and must never be enabled
    # in production.
    ...
    # Clerk session tokens include 'azp' (authorized party). Reject tokens that
    # are issued for an unexpected party, and in production require either azp
    # or a matching Origin/Referer header.
    azp = claims.get("azp")
