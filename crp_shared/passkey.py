...
class PasskeyManager:
    ...
    async def verify_registration(
        ...
        challenge_b64 = bytes_to_base64url(parsed_cred.response.client_data.challenge)
        ...
        await self._record_event(
            ...
            context=AuthContext(ip_address=None, user_agent=None),
            success=True,
            risk_score=0,
    ...
    async def authentication_options(
        ...
        if user_id:
            ...
            allow_credentials = [
                {"type": "public-key", "id": base64url_to_bytes(row["credential_id"])}
                for row in rows
                if not row["revoked"]
            ]
    ...
    async def verify_mfa_session(
        ...
        # Context drift: if IP or UA changed materially, force a fresh passkey.
        factors = []
    ...
    async def assess_risk(self, user_id: str, context: AuthContext) -> RiskAssessment:
        ...
        recent_events = await self.pool.fetch(
            """
            SELECT ip_hash, ua_hash, geo_hash, success, created_at
        ...
        known_geos = {row["geo_hash"] for row in recent_events if row["success"] and row["geo_hash"]}

        if recent_events:
            if context.ip_hash() not in known_ips:
                score += 40.0
                factors.append("unknown_ip")
            if context.ua_hash() not in known_uas:
                score += 30.0
                factors.append("unknown_device")
            if context.geo_hash() and context.geo_hash() not in known_geos:
                score += 25.0
                factors.append("unknown_location")
        else:
            # First ever passkey use
            score += 20.0
        ...
        last_success = next(
            (row["created_at"] for row in recent_events if row["success"]), None
        )
    ...
    async def cleanup_expired(self) -> None:
        ...
        now = time.time()
        await self.pool.execute(
            "DELETE FROM passkey_challenges WHERE expires_at < $1", now
        )
        await self.pool.execute(
            "DELETE FROM passkey_mfa_sessions WHERE expires_at < $1", now
        )
