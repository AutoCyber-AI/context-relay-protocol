...
- **Architectural (the biggest win, but requires the right model).** **Multi-head Latent Attention (MLA)**, used by the DeepSeek architecture, compresses the KV footprint by ~98% vs standard multi-head attention. In llama.cpp: `-mla 3`. This is why DeepSeek-class models punch far above their weight on context. ([spheron.network][22])
